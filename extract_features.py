...
train_dataloader = torch.utils.data.DataLoader(train, batch_size = batch_size, shuffle=False, 
                                               collate_fn=collator_for_lists,num_workers=no_workers,pin_memory=pin_memory,
...
for index,i in enumerate(train_dataloader):
    ...
    for index,i in enumerate(i["idx"]):
        array.append((i,features_cpu[index]))
    j=j+1
    if j%100==0:
...
with open(f"/home/s2765918/code-et/code/npy_data/{data_to_use}_all_20_vit16_new_with_idx.pickle", 'wb') as f:
    pickle.dump(array, f)

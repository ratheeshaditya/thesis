...
def train_model(embed_dim, epochs):
    ...
    for fold, (train_ids, val_ids) in enumerate(kf.split(train.trajectory_ids()), 1):
        ...
        if cfg['MODEL']['MODEL_TYPE'] == 'temporal':
            ...
            kernel = tuple(map(int, cfg['TUBELET']['KERNEL'].split(',')))
            model = TubeletTemporalPart_mean_chan_1_Transformer(dataset=dataset, embed_dim=embed_dim, num_frames=segment_length, num_classes=num_classes, num_joints=num_joints, in_chans=in_chans, kernel=kernel, mlp_ratio=2., qkv_bias=True, qk_scale=None, dropout=0.1)

...
parser.add_argument("--fold_number", help="fold to train on", type=int)
...
def train_model(embed_dim, epochs):
    ...
    if dataset == "HR-Crime":
        file_name_train = '/data/s3447707/MasterThesis/training_results/' + model_name + '_fold_' + str(fold) + '.csv'
        file_name_test = '/data/s3447707/MasterThesis/testing_results/' + model_name + '_fold_' + str(fold) + '.csv'
        num_classes = 13
        num_joints = 17
        num_parts = 5
        in_chans = 2
    elif dataset == "UTK":
        file_name_train = '/data/s3447707/MasterThesis/UTK_training_results/' + model_name + '_fold_' + str(fold) + '.csv'
        file_name_test = '/data/s3447707/MasterThesis/UTK_testing_results/' + model_name + '_fold_' + str(fold) + '.csv'
        num_classes = 10
        num_joints = 20
        in_chans = 3
    ...
    with open(file_name_train, 'w') as csv_file_train:
        ...
        with open(file_name_test, 'w') as csv_file_test:
            ...
            train_ids = np.load(filename_train_ids)
            val_ids = np.load(filename_val_ids)
            ...
            '''
            for i in train_ids:
                row_data = [traj_categories_train[i], traj_videos_train[i], traj_persons_train[i], traj_frames_train[i], X_train[i]]
                print(row_data)
            '''
    ...
    print("Training results of fold {} saved to {}".format(fold, file_name_train))
    print("Testing results of fold {} saved to {}".format(fold, file_name_test))

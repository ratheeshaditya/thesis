    ...
    for h in range(8):
        ...
        if 'ttspcc2' in model_type:
            ...
            for i in range(len(part_labels)):
                ...
                #rearrange scores
                print('spatial_scores shape', spatial_scores.shape)
                # spatial_scores = rearrange(spatial_scores, '(b f) h j1 j2  -> b f h j1 j2', f=num_frames) # COMMENTED
                print('spatial_scores shape', spatial_scores.shape)
                ...
                # draw_spatial(spatial_scores[test_index,frame,h].data, ax=axs_spatial) # COMMENTED
                draw_spatial(spatial_scores[test_index,h].data, ax=axs_spatial)

                image_name = test_sample + '_index_' + str(test_index) + '_layer_' + str(layer+1) + '_head_' + str(h+1) + '_' + part_labels[i] + '_score_' + model_type + '_transformer.jpg'
                image_name = os.path.join(image_location, image_name)
                
                if part_labels[i] == 'torso':
                    step = 2
                else:
                    step = 2 #remaining body parts consist of only two joints
                labels = np.arange(0, input_length, step)
                plt.xticks(np.arange(0, input_length, step)+0.5, np.arange(0, input_length, step))
                plt.yticks(np.arange(0, input_length, step)+0.5, np.arange(0, input_length, step))
